...
<html class="no-js" lang="en">
    ...
    <head>
        ...
        <link
            href='https://api.tiles.mapbox.com/mapbox.js/plugins/leaflet-locatecontrol/v0.24.0/L.Control.Locate.css'
            rel='stylesheet' />
        <script type="text/javascript">
            ...
            $(window).load(function () {
                ...
                MAP = L.mapbox.map('map', 'calecomaps.iggm72f9')
                    .setView([33.984, -118.093], 10).addControl(L.mapbox
                        .geocoderControl('calecomaps.iggm72f9', {
                            keepOpen: false
                        }));
                L.control.locate({
                    locateOptions: {
                ...
                L.control.layers({
                    'Base Map': L.mapbox.tileLayer(
                        'calecomaps.iggm72f9').addTo(MAP),
                }, {
                    'Sensitive Population Density': L.mapbox
                        .tileLayer('calecomaps.7mig8k4v'),
                    'California Protected Area': L.mapbox
                        .tileLayer('calecomaps.toqu8nzc'),
                    'Schools and Colleges': L.mapbox.tileLayer(
                        'calecomaps.vavvh43i'),
                }, {
                    collapsed: false
                }).addTo(MAP);
                MAP.legendControl.addLegend(document.getElementById(
                    'legend-content').innerHTML).addLegend(document
                    .getElementById('legend-content2').innerHTML);
                ...
                MAP.featureLayer.on('click', function (e) {
                    ...
                    var trif_id = e.layer.feature.properties[
                        "TRIF ID"];
            ...
            function drawVisualization(trif_id) {
                ...
                $.get("https://www.googleapis.com/fusiontables/v1/query", {
                    sql: "SELECT * FROM " + TQRG + " WHERE TRIF_ID='" +
                        trif_id + "'",
                    key: apiKey
                }, function (response) {
                    ...
                    data.addColumn('number', 'LA County ' + Industry +
                        ' Industry Average');
                    data.addColumn('number', 'CA ' + Industry +
                        ' Industry Average');
                    data.addColumn('number', 'US ' + Industry +
                        ' Industry Average');
                    data.addRows([
                        [String(QR2010), Number(
                                toxicReleased2010), LaAVG2010,
                            CAAVG2010, USAVG2010
                        ],
                        [String(QR2011), Number(
                                toxicReleased2011), LaAVG2011,
                            CAAVG2011, USAVG2011
                        ],
                        [String(QR2012), Number(
                                toxicReleased2012), LaAVG2012,
                            CAAVG2012, USAVG2012
                        ],
                    ...
                    ac.draw(data, {
                        title: Name +
                            ' Total Toxic Releases On and Off Site',
                        width: 600,
                        height: 400,
                        vAxis: {
                            title: "Total Quantity Released (lbs)",
                            logScale: false
                        },
                        hAxis: {
                            title: "Year"
            ...
            function drawVisualization2(trif_id) {
                $.get("https://www.googleapis.com/fusiontables/v1/query", {
                    sql: "SELECT * FROM " + PWMP + " WHERE TRIF_ID='" +
                        trif_id + "'",
                    key: apiKey
                }, function (response) {
                    var columns = response.columns;
                    ...
                    data.addColumn('number',
                        'Percentage of  Waste Management Practices');
                    data.addColumn('number', 'LA County ' + Industry +
                        ' Industry Average');
                    data.addColumn('number', 'California ' + Industry +
                        ' Industry Average');
                    data.addRows([
                        [String(PWX2010), Number(PPWMP2010),
                            Number(LAAVG2010), Number(CAAVG2010)
                        ],
                        [String(PWX2011), Number(PPWMP2011),
                            Number(LAAVG2011), Number(CAAVG2011)
                        ],
                        [String(PWX2012), Number(PPWMP2012),
                            Number(LAAVG2012), Number(CAAVG2012)
                        ],
                    ...
                    // Create and draw the visualization.
                    var ac = new google.visualization.ComboChart(
                        document.getElementById('PWMPV'));
                    ac.draw(data, {
                        title: Name +
                            ' Percent of Waste Managed Through Recycling, Energy Recovery, and  Treatment',
                        width: 600,
                        height: 400,
                        vAxis: {
                            format: '#%',
                            title: "% Waste Managed Out of Total Production Waste"
                        },
                        hAxis: {
                            title: "Year"
                        },
            ...
            function drawVisualization3(trif_id) {
                $.get("https://www.googleapis.com/fusiontables/v1/query", {
                    sql: "SELECT * FROM " + PIEC + " WHERE TRIF_ID='" +
                        trif_id + "'",
                    key: apiKey
                }, function (response) {
                    var columns = response.columns;
                    ...
                    var options = {
                        title: '2012 ' + Name +
                            ' Share (Pounds) of Total Toxic Releases in the ' +
                            Industry +
                            ' Industry in the LA County',
                        slices: {
                            1: {
                                offset: 0.1,
                                color: String(OthColor),
                                textStyle: {
                                    color: 'transparent'
                                }
                            },
            ...
            function drawVisualization4(trif_id) {
                $.get("https://www.googleapis.com/fusiontables/v1/query", {
                    sql: "SELECT * FROM " + Stats + " WHERE TRIF_ID='" +
                        trif_id + "'",
                    key: apiKey
                }, function (response) {
                    var columns = response.columns;
                    ...
                    $('#CompanyName').html(
                        "<h2 id=\"Name\"><span class=\"subheading\">Facility Name:</span> " +
                        Name + "</h2>");
                    $('#SCORE').html(
                        "<h2 id=\"Score\"><span class=\"subheading\">Environmental Impact Score:</span> " +
                        Math.round(Score) +
                        " out of 100 <a href='eismethodology.php' target='_blank'>[?]</a></h2>"
                    );
                    $('#TypeIndustry').html(
                        "<h2 id=\"industry\"><span class=\"subheading\">Industry NAICS:</span> " +
                        Industry +
                        "</h2>");
                    $('#Note').html(
                        "<h2 id=\"Note\"><span class=\"subheading\">Graphs: When the year is displayed as N/A, the facility did not report to the TRI for that year.</span> </h2>"
                    );
                    $('#noteimpact').html(
                        "<h3 id=\"backtop\">A higher score implies a greater impact on the environment.</h3>"
                    );
                    $('#top').html(
                        "<h3 id=\"backtop\"><a href=#>Back to top</a></h3>"
                    );
                    $('#top2').html(
                        "<h3 id=\"backtop2\"><a href=#>Back to top</a></h3>"
                    );
                    ...
                    var data = google.visualization.arrayToDataTable([
                        ...
                        [
                            'Total Toxic Releases, On- and Off-Site <a href="TOTALTOXICRELEASES.php " target="_blank">[?]</a>',
                            Number(TQR) + ' pounds'
                        ],
                        ['Toxicity of Total Releases,On-Site',
                            TXP + ' toxicity x pounds'
                        ],
                        ['Toxic Releases per $1000 of Revenue<a href="RPKR.php" target="_blank">[?]</a>',
                            Number(Revenue).toFixed(5) +
                            ' pounds/$1000'
                        ],
                        ['Toxic Releases per $1000 of Revenue, Los Angeles Average',
                            Number(RevenueAVG) +
                            ' pounds/$1000'
                        ],
                        [
                            'Waste Managed through Recycling, Energy Recovery, and Treatment<a href="PPWMA.php" target="_blank">[?]</a>',
                            ((PWMP) * 100).toFixed(2) + '%'
                        ],
                        [
                            'Regional Contribution to Lifetime Cancer Risk from Air Emissions<a href="CancerMethod.php" target="_blank">[?]</a>',
                            Number(CANCER).toFixed(2) +
                            ' cancers per million'
                        ],
                        ['California Protected Areas', CPA +
